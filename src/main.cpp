...
int main() {
  ...
  h.onMessage([&ref_vel, &map_waypoints_x, &map_waypoints_y, &map_waypoints_s,
               ...
               &my_lane](uWS::WebSocket<uWS::SERVER> ws, char *data,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          vector<bool> lane_is_empty = {true, true, true};
          lane_is_empty[my_lane] = false;

          for (int i = 0; i < sensor_fusion.size(); i++) {
            // Collect opponent car data
            float d = sensor_fusion[i][6];
            double vx = sensor_fusion[i][3];
            double vy = sensor_fusion[i][4];
            double opponent_speed = sqrt(vx * vx + vy * vy);
            double opponent_s = sensor_fusion[i][5];
            double opponent_lane = getLaneFromFrenet(d);

            // if using previous points project s value out
            opponent_s += ((double)prev_size * .02 * opponent_speed);
          ...
          // if previous size is almost empty, use the car as the starting
          // reference
          if (prev_size < 2) {
          ...
          vector<double> next_wp0 =
              getXY(car_s + 30, (2 + 4 * my_lane), map_waypoints_s,
                    map_waypoints_x, map_waypoints_y);
          vector<double> next_wp1 =
              getXY(car_s + 60, (2 + 4 * my_lane), map_waypoints_s,
                    map_waypoints_x, map_waypoints_y);
          vector<double> next_wp2 =
              getXY(car_s + 90, (2 + 4 * my_lane), map_waypoints_s,
                    map_waypoints_x, map_waypoints_y);
